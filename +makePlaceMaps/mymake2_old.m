function [rateMap, binRangeX, binRangeY, gaussFit2, occupMap] = mymake2(spTrains, videoX, videoY, videoTS, opt)
%[rateMap, binRange, gaussFit1] = mymake2(spTrains, videoX, videoTS, rangeLeft, rangeRight, nBins)
%
...
% Modified by @author Sia
% @date 12/22/2015 01:19 AM
% @lines occupMap no longer logical --> double, as time map
...
smoothingKernel = 1;
dt = (diff(videoTS));
dt = mean(dt(dt < 0.1)); % esimated sampling frequency
...
sem = std(a./b/dt) ./ sqrt(b);w = (1./sem) / sum(1./sem); wn = w*length(w);
bincount_occup(bincount_occup > 0) = (bincount_occup(bincount_occup > 0) - 0) * dt; % expected value of time spent in bin
...
% If occupancy in a bin is less than a certain amount (DEFAULT: median of occupancy time of all visited bins)
% disregard that bin's spikes (and regard its rate as 0).
if ~isfield(opt, 'rateInflationOccupCutoff')
	opt.rateInflationOccupCutoff = prctile(bincount_occup(bincount_occup>=dt), 50); % seconds
end
rmUnsmoothed(bincount_occup<=opt.rateInflationOccupCutoff) = 0;

% rateMap = smooth2a(rmUnsmoothed, smoothingKernel);
rateMap = dataanalyzer.makePlaceMaps.ext.gsmooth(rmUnsmoothed,[5, 5], smoothingKernel); % Chris's smoothing method

if nargout > 3
	rateMapFIT = (bincount_spikes./bincount_occup);
	rateMapFIT(isnan(rateMapFIT)) = 0;
	rateMapFIT = smooth2a(rateMapFIT, smoothingKernel);
	rateMapFIT = dataanalyzer.makePlaceMaps.ext.gsmooth(rateMapFIT,[5, 5], smoothingKernel); % Chris's smoothing method
	...
	try
		...
		warning('DataAnalyzer:PlaceMap:UnsuccessfulFit', 'Unsuccessful fitting.');
		gaussFit2 = [];
	end
end
