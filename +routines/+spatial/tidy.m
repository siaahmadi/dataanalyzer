function [x, y] = tidy(x, y, smoothingWindow)
...
% Modified to run as a standalone version of @dataanalyzer for Silvia
% 2/29/2016 11:18 AM by @author Sia
...
if isnan(x(1)) % this will reduce ridiculous values introduced by interpolation
	x(1) = x(find(isfinite(x), 1));
end
if isnan(y(1)) % this will reduce ridiculous values introduced by interpolation
...
obj.X = conv(x, kernel, 'same'); % MATLAB's @smooth function cannot handle some inputs for some reason (i.e. takes forever)
